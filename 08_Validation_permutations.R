...
# Helper function (QGG)
run_prediction <- function(data_subset_updated, fixed_model, random_model, GRMs, validate_list) {
  ...
  model <- tryCatch(
    {
      greml(y = y, X = X_input, GRM = GRMs, validate = validate_list, ncores = cores)
    },
    error = function(e) {
...
# Main function
Validation_perm <- function(data_list, G_GRM, validation_scheme) {
  final_output <- data.frame()

  P_GRM_list <- readRDS(paste0(path_input, "kinships/random_effects/GRMs_DA_permuted.rds"))

  loads <- readRDS(paste0(path_input, "kinships/fixed_effects/GW_load_DA_permuted.rds"))
  ...
  for (current_trait in names(data_list)) {
    ...
    for (current_permute in names(P_GRM_list)) {
      P_GRM_list_upd <- P_GRM_list[[current_permute]]

      for (current_interval in names(P_GRM_list_upd)) {
        P_GRM <- P_GRM_list_upd[[current_interval]][["P"]]

        loads_upd <- loads %>%
          dplyr::filter(permutation_seed == current_permute, q_interval == current_interval) %>%
          select(c("id", "G", "P"))
        ...
        V_list <- list("G" = G_GRM_ext, "P" = P_GRM[data_subset$id, data_subset$id])
        ...
        output <- bind_rows(
          ...
          mutate(
            "trait" = current_trait, "validation_scheme" = validation_scheme,
            "permuted" = "yes", "permutation" = current_permute, "q_interval" = current_interval
          )
    ...
    saveRDS(final_output, paste0(path_output, "validation/validation_", validation_scheme, "_permuted_intermediate_", submission, ".rds"))
  }

  saveRDS(final_output, paste0(path_output, "validation/validation_", validation_scheme, "_permuted_", submission, ".rds"))
}
...
Validation_perm(data_list, G_GRM, "genetic_cluster")
